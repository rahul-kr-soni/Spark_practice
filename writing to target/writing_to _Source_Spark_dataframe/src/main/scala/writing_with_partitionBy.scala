...
object writing_with_partitionBy extends App {
  ...
  val orderRep = orderdf.repartition(1)
  ...
  orderRep.write
    ...
    .option("maxRecordsPerFile",2000)  // use to having specific number of rows per file
    .option("path", "G:\\scalalearning\\writing_to _Source_Spark_dataframe\\output_source\\partition_By")
    .save()
